...
void DestModel::load()
{
    ...
    connect(m_request, &KCupsRequest::deviceMap, this, &DestModel::createItem);
    connect(m_request, &KCupsRequest::finished, this, &DestModel::finished);
    m_request->getDestinations();
...
void DestModel::finished()
{
    bool hasError = m_request->hasError();
    if (hasError) {
        qCDebug(LIBKCUPS) << "ERROR" << m_request->errorMsg();
