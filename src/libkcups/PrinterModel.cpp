...
{
    Q_ASSERT(printer != m_printers.end());
    if (printer->isClass()) {
        return;
    }

    QStringList attr = QStringList{KCUPS_PRINTER_MORE_INFO, KCUPS_PRINTER_SUPPLY_INFO_URI, KCUPS_PRINTER_ICONS};
...
{
    auto p = printer;
    /** If not a class, try to resolve the URI, which might originally
     *  configured as a DNSSD search uri
    */
    if (!p.isClass()) {
        const auto uri = PrinterCommands::resolveToUri(p.deviceUri());
        if (!uri.isEmpty()) {
            qCDebug(LIBKCUPS) << p.name() << "URI Resolved:" << p.deviceUri() << "->" << uri;
            p.setAttribute(KCUPS_DEVICE_URI, uri);
        }
    }
    // if item found, update it, otherwise add to model
    if (const auto ndx = findIndex(p.name()); ndx == -1) {
        qCDebug(LIBKCUPS) << "Model ADDING" << p.name();
        const auto row = m_printers.size();
        beginInsertRows(QModelIndex(), row, row);
        m_printers.append(p);
        endInsertRows();
    } else {
        qCDebug(LIBKCUPS) << "Model UPDATING" << p.name() << ndx;
        m_printers[ndx] = p;
