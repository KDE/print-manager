...
public:
    explicit PrintQueue(const QString &dest, QObject *parent = nullptr);
    ~PrintQueue() override;

    QQuickWindow *mainWindow() const;

public Q_SLOTS:
    void init(const QString &destName);
    void authenticateJob(const QString &destName, int jobId, bool isClass);
