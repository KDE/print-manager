...
void KCupsRequest::getPPD(const QString &printerName){
    if(m_connection->readyToStart()){
        do{
            ...
            if (num_dests > 0 && dests) {
                cups_dest_t *dest = cupsGetDest(printerName.toUtf8().constData(), nullptr, num_dests, dests);

                if (dest) {
                    cups_dinfo_t *dinfo = cupsCopyDestInfo(CUPS_HTTP_DEFAULT, dest);
                    if (dinfo) {
                        const char *ppdFile = cupsGetOption("ppd-file", dest->num_options, dest->options);

                        if (ppdFile) {
                            m_ppdFile = QString::fromUtf8(ppdFile);
                            qCDebug(LIBKCUPS) << "PPD File Path:" << m_ppdFile;
                        } else {
                            qCWarning(LIBKCUPS) << "PPD file not found in printer options.";
                        }

                        cupsFreeDestInfo(dinfo);
                    } else {
                        qCWarning(LIBKCUPS) << "Failed to retrieve printer destination info.";
                    }
                } else {
                    qCWarning(LIBKCUPS) << "Printer destination not found.";
                }

                cupsFreeDests(num_dests, dests);
            } else {
                qCWarning(LIBKCUPS) << "Failed to retrieve printer destinations.";
            }
        } while (m_connection->retry("/", CUPS_GET_PPD));
        ...
        invokeMethod("getPPD", printerName);
    }
}
