...
void MarkerLevelChecker::checkMarkerLevels(const QString &printerName)
{
    ...
    /**
     *  Per OpenPrinting, marker low level boundary (val==3) indicates the near
     *  empty warning value, but some printers will actually report 0.
     *  If it is 0, use threshold as the low-value check.
     *
    ...
    connect(request, &KCupsRequest::finished, this, [this](KCupsRequest *req) {
        ...
        if (!req->printers().isEmpty()) {
            ...
            if (!currentLevels.isEmpty() && !highLevels.isEmpty() && !lowLevels.isEmpty() && !typesList.isEmpty() && !namesList.isEmpty()) {
                ...
                qCDebug(PMKDED) << "At least one marker level attribute is empty, aborting level check" << Qt::endl
                                << "Current:" << currentLevels << Qt::endl
                                << "Low:" << lowLevels << Qt::endl
                                << "High:" << highLevels << Qt::endl
                                << "Names:" << namesList << Qt::endl
                                << "Types:" << typesList;
            }
