...
void PrinterModel::newDevice(const QVariantMap &device)
{
...
void PrinterModel::update()
{
    ...
    connect(request, &KCupsRequest::deviceMap, this, &PrinterModel::newDevice);
...
void PrinterModel::updateDest(QStandardItem *destItem, const KCupsPrinter &printer)
{
    ...
    if (markerChangeTime != destItem->data(DestMarkerChangeTime)) {
        ...
        const QVariantMap markers{{KCUPS_MARKER_CHANGE_TIME, printer.markerChangeTime()},
                                  {KCUPS_MARKER_COLORS, printer.attribute(KCUPS_MARKER_COLORS)},
                                  {KCUPS_MARKER_LEVELS, printer.attribute(KCUPS_MARKER_LEVELS)},
                                  {KCUPS_MARKER_NAMES, printer.attribute(KCUPS_MARKER_NAMES)},
                                  {KCUPS_MARKER_TYPES, printer.attribute(KCUPS_MARKER_TYPES)}};
...
{
    qCWarning(LIBKCUPS) << "printerRemoved" << text << printerUri << printerName << printerState << printerStateReasons << printerIsAcceptingJobs;
    ...
    if (row != -1) {
        ...
        qCWarning(LIBKCUPS) << "Unable to remove printer: not in model" << printerName;
    }
    updateDisplayHints();
}
                                       ...
                                       bool printerIsAcceptingJobs)
{
    ...
    if (QVersionNumber(CUPS_VERSION_MAJOR, CUPS_VERSION_MINOR, CUPS_VERSION_PATCH) < QVersionNumber(2, 4, 13)) {
        ...
        int row = destRow(printerName);
        if (row != -1) {
            item(row)->setData(printerState == KCupsPrinter::Stopped || !printerIsAcceptingJobs, DestIsPaused);
            ...
            qCWarning(LIBKCUPS) << "Unable to set State, printer not in model:" << printerName;
        }
    }
}
