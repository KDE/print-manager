...
PrinterCommands *PrinterCommands::instance()
{
...
void PrinterCommands::savePrinter(const QString &name, const QVariantMap &saveArgs, bool isClass)
{
    ...
    qCDebug(LIBKCUPS) << (addMode ? "New Printer:" : "Change Printer:") << name << Qt::endl
                      << "isClass?" << isClass << Qt::endl
                      << "Changing Default?" << isDefault << Qt::endl
...
{
    const auto request = setupRequest(&PrinterCommands::testDone);
...
{
    const auto request = setupRequest(&PrinterCommands::cleanDone);
...
{
    const auto request = setupRequest(&PrinterCommands::pauseDone);
    request->pausePrinter(printerName);
}

void PrinterCommands::removePrinter(const QString &printerName)
{
    const auto request = setupRequest(&PrinterCommands::removeDone);
    request->deletePrinter(printerName);
}

void PrinterCommands::resumePrinter(const QString &printerName)
{
    const auto request = setupRequest(&PrinterCommands::resumeDone);
    request->resumePrinter(printerName);
}

KCupsRequest *PrinterCommands::setupRequest(SignalFunc finished_cb, RequestFunc error_cb)
{
    auto request = new KCupsRequest;
    connect(request, &KCupsRequest::finished, this, [this, finished_cb, error_cb](KCupsRequest *r) {
        if (r->hasError()) {
            if (error_cb) {
                (this->*error_cb)(r);
            } else {
...
KCupsRequest *PrinterCommands::setupRequest(RequestFunc finished_cb, RequestFunc error_cb)
{
    auto request = new KCupsRequest;
    connect(request, &KCupsRequest::finished, this, [this, finished_cb, error_cb](KCupsRequest *r) {
        if (r->hasError()) {
...
KCupsRequest *PrinterCommands::setupRequest(StdRequestFunc finished_cb, StdRequestFunc error_cb)
{
    auto request = new KCupsRequest;
    connect(request, &KCupsRequest::finished, this, [this, finished_cb, error_cb](KCupsRequest *r) {
        if (r->hasError()) {
...
void PrinterCommands::defaultErrorCB(KCupsRequest *request)
{
