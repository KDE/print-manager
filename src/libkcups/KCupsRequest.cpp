...
static int get_dest_cb(void *user_data, unsigned flags, cups_dest_t *dest)
{
    ...
    const auto toIntList = [](const QString &option) -> QVariant {
        QList<int> ret;
    ...
    if (map.value(KCUPS_MARKER_CHANGE_TIME).toInt() > 0) {
        ...
        map.insert(KCUPS_MARKER_LEVELS, toIntList(map.value(KCUPS_MARKER_LEVELS).toString()));
        ...
        qCDebug(LIBKCUPS) << "Markers from CUPS:" << pname << Qt::endl
                          << map.value(KCUPS_MARKER_TYPES) << Qt::endl
                          << map.value(KCUPS_MARKER_NAMES) << Qt::endl
                          << map.value(KCUPS_MARKER_COLORS) << Qt::endl
                          << map.value(KCUPS_MARKER_LEVELS);
    }
    ...
    static_cast<KCupsRequest *>(user_data)->addPrinter(map);
    return (1);
}

void KCupsRequest::addPrinter(const QVariantMap &device)
{
    m_printers << KCupsPrinter(device);
}
