...
PrintQueue::PrintQueue(const QString &dest, QObject *parent)
    : QObject(parent)
    , m_engine(new QQmlApplicationEngine)
{
    ...
    QTimer::singleShot(200, this, [this, dest]() {
        Q_EMIT showQueue(dest);
    });
}
...
void PrintQueue::init(const QString &destName)
{
    m_mainWindow->show();
    Q_EMIT showQueue(destName);
}

void PrintQueue::authenticateJob(const QString &destName, int jobId, bool isClass)
{
    const auto authRequest = new KCupsRequest;
    connect(authRequest, &KCupsRequest::finished, this, [this, destName, jobId](KCupsRequest *request) {
        request->deleteLater();

        if (request->hasError() || request->printers().size() != 1) {
            qCWarning(PMQUEUE) << "Ignoring request, printer not found or error" << destName << request->errorMsg();
            return;
        }
        ...
        info.prompt = i18n("Enter credentials to print from <b>%1</b>", destName);
        ...
        authRequest2->authenticateJob(destName, authInfo, jobId);
    });

    authRequest->getPrinterAttributes(destName, isClass, {KCUPS_PRINTER_URI_SUPPORTED, KCUPS_AUTH_INFO_REQUIRED});
}
