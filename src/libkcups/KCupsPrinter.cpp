...
QString KCupsPrinter::iconUri() const
{
    const auto iconList = m_attributes.value(KCUPS_PRINTER_ICONS).toList();
    // Fallback to the generic icons if nothing is available from the device
    if (iconList.size() == 0) {
        return iconName();
    }
    // Quality/size of the images is from smallest (0) to largest (n)
    return iconList.size() > 1 ? iconList.at(1).toString() : iconList.at(0).toString();
}
